...
Data set(s) must contain: 

Depth, method, year, (location), and at least one each of the following metals 

As, Cd, Pb, Hg, Ni, Tl, Cr 

 
Places searched for data: USGS and Arctic Data Center 

Nothing found in Arctic Data Center 

All downloads done on 2/21/2019 


Data filtering methods:  

Loaded Dataset: USGS Alaskan Geochemical Soil Database -> “BestValue_Soil.xlxs” on 2/21/2019.  Converted to .csv file for loading into R studio version 3.5.2, Eggshell Igloo. 

Loaded tidyverse, dplyr, RCurl, ggplot2, reshape2 and agricolae packages (add version numbers?)

Set script to read in csv files from github repository = https://raw.githubusercontent.com/jagilman/Arctic-Ecol-Soc-2019/master/BestValue_Soil.csv 

Read in csv, took out first 4 rows that were blank, indicted there was a header, set all missing values to “NA” 

Filtered out columns that were redundant chemical concentration and analytical method info using “ends_with” command (columns reduced from 326 to 229) 

Selected columns of desired chemistry data and sample info using select(data, matches())  

Kept chemistry columns that start with As_, Cd_, Pb_, Hg_, Ni_, Cr_, and Tl_ to get both concentration and analytical methods 

Kept lab ID, field ID, data collected/submitted, latitude/longitude, quad, spheroid, datum, sample source, depth, primary class, location description, sample comment, sample zone, horizon, organics, drainage, and prep columns to capture relevant sample, sampling, and processing information 

Columns reduced from 229 to 34 

Filtered out samples that had missing latitude, longitude, depth, and sample source information 

Rows reduced from 6869 to 4449 

All samples have lat/lon, depth, and sample source and concentration information

Filtering everything out that had no info on depth and sample source

Then narrowed down columns.  

We need to wrangle the depths.... 
Recoded depths to get all into a similar format, for example surface (0-25cm) became 0-25 cm. Then filtered depths matching categorical variables: unknown, by horizon, various, A horizon, B horizon, C horizon, surface, Various, loess, loess ?.  Depth units were converted to all have a consistent format, for example inches became in and feet became ft.  The depth column was separated into two columns, depth and units.  Depth column was then sepearated unto upper and low units.  Rows that could not be split and/or parsed correctly were removed.  Database was then filtered to keep only rows that contained units.  
...
Created .csv file and uploated to github repository.  Pulled filtered_USGS.csv file from repository into R Studio.  Sources that had only one sample were removed (there were only two of these).  Some analytical methods and sampling sites had identical values for As, Cr, Pb, and Ni, these data were deemed erronous and removed.  Samples with negative Pb values were removed.  
...
The Shapiro test was used to check for normality.  The Bartlett test was used to check for homogeneity of variance.  Kruskal-Wallis was used in lieu of ANOVA due to lack of normality for all heavy metals in this dataset.  Bonferroni was then used as a p-value adjustment method to make the significant of differences more concervative.  
...
Attempted oridination plot, however, missing valuees had to be removed.  Log transformed clusters all in one corner where as w/o log transform, points are scattered with no clear grouping/separating out by source. ()
