...
Now we need to convert units all to cm...work in progress...meanwhile I am going to do it offline so we can move forward. We can fix this part later
...
```{r}
boxplot(USGS_numeric$As_ppm)
dotchart(USGS_numeric$As_ppm, xlab = "As (ppm)", xlim=c(-50,400),
         ylab = "Order of the data")

boxplot(USGS_numeric$Cr_ppm)
dotchart(USGS_numeric$Cr_ppm, xlab = "Cr (ppm)", xlim=c(-50,400),
         ylab = "Order of the data")

boxplot(USGS_numeric$Hg_ppm)
dotchart(USGS_numeric$Hg_ppm, xlab = "Hg (ppm)", xlim=c(-50,200),
         ylab = "Order of the data")

boxplot(USGS_numeric$Ni_ppm)
dotchart(USGS_numeric$Ni_ppm, xlab = "Ni (ppm)", xlim=c(-50,400),
         ylab = "Order of the data")

boxplot(USGS_numeric$Pb_ppm)
dotchart(USGS_numeric$Pb_ppm, xlab = "Pb (ppm)", xlim=c(-50,4000),
...
USGS_numeric$As_ppm <- as.numeric(USGS_numeric$As_ppm)
USGS_numeric$Cr_ppm <- as.numeric(USGS_numeric$Cr_ppm)
USGS_numeric$Ni_ppm <- as.numeric(USGS_numeric$Ni_ppm)
USGS_numeric$Hg_ppm <- as.numeric(USGS_numeric$Hg_ppm)
