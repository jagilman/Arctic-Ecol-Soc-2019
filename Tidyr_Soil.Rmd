...
limits <- c(mean(USGS_analyze$As_ppm > 32, na.rm=TRUE), mean(USGS_analyze$Cr_ppm > 100, na.rm=TRUE), mean(USGS_analyze$Ni_ppm > 35, na.rm=TRUE), mean(USGS_analyze$Pb_ppm > 85, na.rm=TRUE))

#making a dataframe to add metals names
limits <- as.data.frame(limits)
limits$metals <- c("Arsenic", "Chromium", "Nickel", "Lead")
barplot(limits$limits)

#start working on plot here! 
ggplot(limits, aes(metals, limits)) +
  geom_linerange(
    aes(x = metals, ymin = 0, ymax = limits), 
    color = "lightgray", size = 1.5
    )+
  geom_point(aes(color = metals), size = 2)+
  ggpubr::color_palette("jco")+ theme_classic()
