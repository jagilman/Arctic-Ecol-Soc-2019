...
```
Making NEW figure for % over health limits and USA averages: 
```{r}
...
PCA? Comparing metals? 
```{r}
##Honestly - I don't think this is informative (because of lack of consistent metadata and/or ancillary environmental data across samples) nor really necessary for the message of the paper? 

### work on new PCA here!
##this isn't working..for some reason its turning the data martix into a time series object? will start working back here next work session. 
pca.matrix <-filter(USGS_analyze, is.na(USGS_analyze$Hg_ppm) == FALSE)
pca.matrix <- filter(pca.matrix, is.na(As_ppm) == FALSE)
pca.matrix <- filter(pca.matrix, is.na(Cr_ppm) == FALSE)
pca.matrix <- filter(pca.matrix, is.na(Ni_ppm) == FALSE)
pca.matrix <- filter(pca.matrix, is.na(Pb_ppm) == FALSE)
#Removing NAs for each metal to ensure each sample has all values reduced sample number from 1551 to 787
library(ggfortify)
main.pca <- (pca.matrix[c(14,16,18,20,22)])
metals.pca <- prcomp(main.pca)
summary(metals.pca)
loadings(metals.pca)
autoplot(metals.pca, data = pca.matrix, colour = "Source", loadings=TRUE, loadings.label=TRUE)


fviz_pca_var(metals.pca,
...
# with mines: 0.1425
mean(no_mine$As_ppm > 32, na.rm=TRUE)
# without mines = 0.1262
...
# with mines: 0.7211
mean(no_mine$As_ppm > 7.2, na.rm=TRUE)
# without mines = 0.7158
...
# with mines: 0.09235075
mean(no_mine$Cr_ppm > 100, na.rm=TRUE)
# without mines: 0.08641975
...
# with mines: 0.7014925
mean(no_mine$Cr_ppm > 37, na.rm=TRUE)
# without mines: 0.697056
...
# with mines: 0.0375
mean(no_mine$Hg_ppm > 6.6, na.rm=TRUE)
# without mines: 0.02284264
...
# with mines 0.935
mean(no_mine$Hg_ppm > 0.029, na.rm=TRUE)
# without mines: 0.934
...
# with mines: 0.3522833
mean(no_mine$Ni_ppm > 35, na.rm=TRUE)
# without mines: 0.3425047
#Calculates the proportion of values greater than the US avg for Ni in soil from the Toxic Substances and Disease Registry (13 mg/kg)
mean(USGS_analyze$Ni_ppm > 13, na.rm=TRUE)
# with mines: 0.8583411
mean(no_mine$Ni_ppm > 13, na.rm=TRUE)
# without mines: 0.8567362

#Calculates the proportion of values greater than the WHO Pb in soil limit (85 mg/kg)
mean(USGS_analyze$Pb_ppm > 85, na.rm=TRUE)
# with mine: 0.02867715
mean(no_mine$Pb_ppm > 85, na.rm=TRUE)
# without mines: 0.02730697
...
# with mines: 0.1008326
mean(no_mine$Pb_ppm > 30, na.rm=TRUE)
# without mines: 0.09510358

#making dataframes for plots
#df1 - has perecent over health limits for sites with/without mines
df1 <- data.frame(metal= rep(c("As", "Cr", "Hg", "Ni", "Pb"), 2),
                  mines= rep(c("with mines", "without"), each = 5),
                  percent = c(mean(USGS_analyze$As_ppm > 32, na.rm=TRUE), mean(USGS_analyze$Cr_ppm > 100, na.rm=TRUE), mean(USGS_analyze$Hg_ppm > 6.6, na.rm=TRUE), mean(USGS_analyze$Ni_ppm > 35, na.rm=TRUE), mean(USGS_analyze$Pb_ppm > 85, na.rm=TRUE), mean(no_mine$As_ppm > 32, na.rm=TRUE), mean(no_mine$Cr_ppm > 100, na.rm=TRUE), mean(no_mine$Hg_ppm > 6.6, na.rm=TRUE), mean(no_mine$Ni_ppm > 35, na.rm=TRUE), mean(no_mine$Pb_ppm > 85, na.rm=TRUE)))
...
df2 <- data.frame(metal= rep(c("As", "Cr", "Hg", "Ni", "Pb"), 2),
                  mines= rep(c("with mines", "without"), each = 5),
                  percent = c(mean(USGS_analyze$As_ppm > 7.2, na.rm=TRUE), mean(USGS_analyze$Cr_ppm > 37, na.rm=TRUE), mean(USGS_analyze$Hg_ppm > 0.029, na.rm=TRUE), mean(USGS_analyze$Ni_ppm > 13, na.rm=TRUE), mean(USGS_analyze$Pb_ppm > 30, na.rm=TRUE), mean(no_mine$As_ppm > 7.2, na.rm=TRUE), mean(no_mine$Cr_ppm > 37, na.rm=TRUE), mean(no_mine$Hg_ppm > 0.029, na.rm=TRUE), mean(no_mine$Ni_ppm > 13, na.rm=TRUE), mean(no_mine$Pb_ppm > 37, na.rm=TRUE)))
...
df3 <- data.frame(metal= rep(c("As", "Cr", "Hg", "Ni", "Pb"), 2),
                  ...
                  percent = c(mean((USGS_analyze$As_ppm > 32)*100, na.rm=TRUE), mean((USGS_analyze$Cr_ppm > 100)*100, na.rm=TRUE), mean((USGS_analyze$Hg_ppm > 6.6)*100, na.rm=TRUE), mean((USGS_analyze$Ni_ppm > 35)*100, na.rm=TRUE), mean((USGS_analyze$Pb_ppm > 85)*100, na.rm=TRUE), mean((USGS_analyze$As_ppm > 7.2)*100, na.rm=TRUE), mean((USGS_analyze$Cr_ppm > 37)*100, na.rm=TRUE), mean((USGS_analyze$Hg_ppm > 0.029)*100, na.rm=TRUE), mean((USGS_analyze$Ni_ppm > 13)*100, na.rm=TRUE), mean((USGS_analyze$Pb_ppm > 30)*100, na.rm=TRUE)))
