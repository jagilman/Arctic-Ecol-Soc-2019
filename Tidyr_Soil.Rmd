...
library(MedOr) #this package calcuates 95% CI of medians
...
#calculates 95% CI for the median of As values
conf.interval(USGS_analyze$As_ppm, alpha=0.95)
...
#calculates 95% CI for the median of Cr values
conf.interval(USGS_analyze$Cr_ppm, alpha=0.95)
...
#calculates 95% CI for the median of Hg values
conf.interval(USGS_analyze$Hg_ppm, alpha=0.95)
...
#caluclates 95% CI for the median of Ni values
conf.interval(USGS_analyze$Ni_ppm, alpha=0.95)
...
#calculates 95% CI for the median of Pb values
conf.interval(USGS_analyze$Pb_ppm, alpha=0.95)
...
Pb_plot + 
  ...
  annotate("text", x = 1:5, y = 10, label = c("b", "b", "b", "a", "c"))
##Line values from WHO permissible limits for heavy metals in plant and soil (1996)
# + geom_hline(yintercept=4.4426, linetype="dashed", size=0.75)
# Adds US Average from Toxic Substances and Disease Registry
# + geom_hline(yintercept=3.401, linetype="dotted", size=0.75) 
# Adds group means
#+ stat_summary(fun.y=mean, geom="point", shape=23, size=3, color="black", fill="white") 
...
#                   mines= rep(c("with", "without"), each = 5),
...
# 
# #df12 - has perecent over US averages for sites with/without mines
...
df3 <- data.frame(metal= rep(c("As", "Cr", "Hg", "Ni", "Pb"), 2),
                  ...
                  percent = c(mean(USGS_analyze$As_ppm > 32, na.rm=TRUE), mean(USGS_analyze$Cr_ppm > 100, na.rm=TRUE), mean(USGS_analyze$Hg_ppm > 6.6, na.rm=TRUE), mean(USGS_analyze$Ni_ppm > 35, na.rm=TRUE), mean(USGS_analyze$Pb_ppm > 85, na.rm=TRUE), mean(USGS_analyze$As_ppm > 7.2, na.rm=TRUE), mean(USGS_analyze$Cr_ppm > 37, na.rm=TRUE), mean(USGS_analyze$Hg_ppm > 0.029, na.rm=TRUE), mean(USGS_analyze$Ni_ppm > 13, na.rm=TRUE), mean(USGS_analyze$Pb_ppm > 30, na.rm=TRUE)))

ggplot(data=df3, aes(x=metal, y=percent, fill=limit)) +
  geom_bar(stat="identity", color="black", position=position_dodge()) + ylim(0,1.0) +
...
OLD VERSION OF DATAFRAME AND PLOT CREATION for % over limits and averages: 
```{r}
...
averages <- c(mean(USGS_analyze$As_ppm > 7.2, na.rm=TRUE), mean(USGS_analyze$Cr_ppm > 37, na.rm=TRUE), mean(USGS_analyze$Hg_ppm > 0.029, na.rm=TRUE), mean(USGS_analyze$Ni_ppm > 13, na.rm=TRUE), mean(USGS_analyze$Pb_ppm > 30, na.rm=TRUE))
