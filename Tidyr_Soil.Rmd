...
USGS4 <- select(USGS3,FIELD_ID, DATE_COLLECT, DEPTH, LOCATE_DESC, HORIZON, DATUM, LATITUDE, LONGITUDE, SAMPLE_SOURCE, As_ppm, As_ppm_AM, Cd_ppm, Cd_ppm_AM, Cr_ppm, Cr_ppm_AM, Ni_ppm, Ni_ppm_AM, Pb_ppm, Pb_ppm_AM) #dropped Tl - no data.
...
#separating the top depth from the interval
USGS5 <- separate(USGS4, col=c("DEPTH"), into=c("UPPER", "DEPTH"), sep="-") 
#separating the lower depth
USGS6 <- separate(USGS5, col=c("DEPTH"), into=c("LOWER", "UNIT"), sep=" ")
#taking out rows without depths
USGS7 <- filter(USGS6, LOWER != "unknown") 
USGS8 <- filter(USGS7, LOWER != "by horizon")
###issue here - several rows where there wasn't a space between Xcm, and some the lower depth and unit didn't split
##ideally - we split the columns on a pattern detection that splits the upper depth out at first non numeric charcter, the lower depth after the next numeric value, and then isolates the unit
```
...
>>>>>>> b52b46dcaf3799bc44a4a117f00f2d5f2e36c629
