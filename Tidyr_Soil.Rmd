...
Making nicers box plots to look at concentration by sample source.
...
As_count <- select(USGS_analyze, Source, TOP_CM, As_ppm)
...
Cr_plot + scale_fill_manual(values=c("#8dd3c7", "#ffffb3", "#bebada", "#fb8072", "#80b1d3", "#fdb462", "#b3de69", "#fccde5", "#bc80bd")) + geom_hline(yintercept=100, linetype="dashed", size=1.25)
##Line values from WHO permissible limits for heavy metals in plant and soil (1996)

Cr_count <- select(USGS_analyze, Source, TOP_CM, Cr_ppm)
Cr_count<- filter(Ni_count, !is.na(Cr_ppm))
Cr_count %>% 
  group_by(Source) %>%
  summarise(no_rows = length(Source))
...
Ni_plot + scale_fill_manual(values=c("#8dd3c7", "#ffffb3", "#bebada", "#fb8072", "#80b1d3", "#fdb462", "#b3de69", "#fccde5", "#bc80bd")) + geom_hline(yintercept=35, linetype="dashed", size=1.25)
##Line values from WHO permissible limits for heavy metals in plant and soil (1996)
...
Pb_plot + scale_fill_manual(values=c("#8dd3c7", "#ffffb3", "#bebada", "#fb8072", "#80b1d3", "#fdb462", "#b3de69", "#fccde5", "#bc80bd")) + geom_hline(yintercept=1.9294, linetype="dashed", size=1.25)
