...
Cleaning data to get depth data in consistent formats and writing out file to manually code depth.  
```{r}
...
OLD % over calculations/plots
```{r}

#Making a dataframe with no mine-impacted soil to compare % levels
no_mine <- filter(USGS_analyze, Source != "mine-impacted")

#making dataframes for plots
#df1 - has perecent over health limits for sites with/without mines. currently has values from Kamunda et al. (2016) for Cananda, who cites "Soil, Ground Water and Sediment Standards for Use Under Part XV.1 of the Environmental Protection Act" from Canada, but does not report the same numbers as in those government documents? 
df1 <- data.frame(metal= rep(c("As", "Cr", "Hg", "Ni", "Pb"), 2),
                  mines= rep(c("with mines", "without"), each = 5),
                  percent = c(mean(USGS_analyze$As_ppm > 20, na.rm=TRUE), mean(USGS_analyze$Cr_ppm > 250, na.rm=TRUE), mean(USGS_analyze$Hg_ppm > 0.8, na.rm=TRUE), mean(USGS_analyze$Ni_ppm > 100, na.rm=TRUE), mean(USGS_analyze$Pb_ppm > 200, na.rm=TRUE), mean(no_mine$As_ppm > 20, na.rm=TRUE), mean(no_mine$Cr_ppm > 250, na.rm=TRUE), mean(no_mine$Hg_ppm > 0.8, na.rm=TRUE), mean(no_mine$Ni_ppm > 100, na.rm=TRUE), mean(no_mine$Pb_ppm > 200, na.rm=TRUE)))

ggplot(data=df1, aes(x=metal, y=percent, fill=mines)) +
...
ylab("proportion of samples over health limits")+
geom_bar(stat="identity", position=position_dodge()) + ylim(0,1.0) + theme_classic()+
theme(axis.title.x=element_blank(),
...
#df2 - has perecent over US averages for sites with/without mines. 
#Currently using averagr values for the A Horizon from the USGS, Smith et al., 2013, Geochem. and Mineralogical Data for Soils of the Conterminous US, Data Series 801
df2 <- data.frame(metal= rep(c("As", "Cr", "Hg", "Ni", "Pb"), 2),
                  mines= rep(c("with mines", "without"), each = 5),
                  percent = c(mean(USGS_analyze$As_ppm > 6.6, na.rm=TRUE), mean(USGS_analyze$Cr_ppm > 37, na.rm=TRUE), mean(USGS_analyze$Hg_ppm > 0.04, na.rm=TRUE), mean(USGS_analyze$Ni_ppm > 18.5, na.rm=TRUE), mean(USGS_analyze$Pb_ppm > 22.2, na.rm=TRUE), mean(no_mine$As_ppm > 6.6, na.rm=TRUE), mean(no_mine$Cr_ppm > 37, na.rm=TRUE), mean(no_mine$Hg_ppm > 0.04, na.rm=TRUE), mean(no_mine$Ni_ppm > 18.5, na.rm=TRUE), mean(no_mine$Pb_ppm > 22.2, na.rm=TRUE)))

ggplot(data=df2, aes(x=metal, y=percent, fill=mines)) +
scale_fill_manual(values=c("#bdbdbd", "#737373"))+
geom_text(aes(label=round(percent, 2)), vjust=-0.2, color="black",
position = position_dodge(0.9), size = 3.25, fontface = "bold") +
ylab("proportion of samples over US average")+
geom_bar(stat="identity", position=position_dodge()) + ylim(0,1.0) + theme_classic()+
theme(axis.title.x=element_blank(),
        axis.title.y=element_text(face="bold"),
        axis.text.x = element_text(size = 10, face = "bold"),
...
#df3 - has percent over health limits/over averages for whole dataset
df3 <- data.frame(metal= rep(c("As", "Cr", "Hg", "Ni", "Pb"), 2),
                  limit=rep(c("health limit","US average"),each = 5),
                  percent = c(mean((USGS_analyze$As_ppm > 20)*100, na.rm=TRUE), mean((USGS_analyze$Cr_ppm > 250)*100, na.rm=TRUE), mean((USGS_analyze$Hg_ppm > 0.8)*100, na.rm=TRUE), mean((USGS_analyze$Ni_ppm > 100)*100, na.rm=TRUE), mean((USGS_analyze$Pb_ppm > 200)*100, na.rm=TRUE), mean((USGS_analyze$As_ppm > 6.6)*100, na.rm=TRUE), mean((USGS_analyze$Cr_ppm > 37)*100, na.rm=TRUE), mean((USGS_analyze$Hg_ppm > 0.04)*100, na.rm=TRUE), mean((USGS_analyze$Ni_ppm > 18.5)*100, na.rm=TRUE), mean((USGS_analyze$Pb_ppm > 22.2)*100, na.rm=TRUE)))

ggplot(data=df3, aes(x=metal, y=percent, fill=limit)) +
  geom_bar(stat="identity", position=position_dodge()) + ylim(0,100) +
  geom_text(aes(label=round(percent, 2)), vjust=-0.2, color="black",
  position = position_dodge(0.9), size = 3.25, fontface = "bold") +
  scale_fill_manual(values=c("#bdbdbd", "#737373"))+
  ylab("Percent of sample locations over limit")+
  theme_classic() +
  theme(axis.title.x=element_blank(),
        axis.title.y=element_text(face="bold"),
        axis.text.x = element_text(size = 10, face = "bold"),
...
# Do we need this? the % show up on the plots so probably not: 
#Calculates the % of values greater than the Canada EPA soil limit reported in Kamunda et al. (2016) (20 mg/kg)
# mean(USGS_analyze$As_ppm > 20, na.rm=TRUE)
# mean(no_mine$As_ppm > 20, na.rm=TRUE)
#Calculates the proportion of values greater than the US avg for A horizon soils from the USGS reported in Smith et al. (2013), Data Series 801 (6.6 mg/kg)
# mean(USGS_analyze$As_ppm > 6.6, na.rm=TRUE)
# mean(no_mine$As_ppm > 6.6, na.rm=TRUE)

# #Calculates the % of values greater than the Canada EPA soil limit reported in Kamunda et al. (2016)  (250 mg/kg)
# mean(USGS_analyze$Cr_ppm > 250, na.rm=TRUE)
# mean(no_mine$Cr_ppm > 250, na.rm=TRUE)
# Calculates the proportion of values greater than the US avg for A horizon soils from the USGS reported in Smith et al. (2013), Data Series 801 (37 mg/kg)
# mean(USGS_analyze$Cr_ppm > 37, na.rm=TRUE)
# mean(no_mine$Cr_ppm > 37, na.rm=TRUE)
# 
# Calculates the % of values greater than the Canada EPA soil limit reported in Kamunda et al. (2016) (0.8 mg/kg)
# mean(USGS_analyze$Hg_ppm > 0.8, na.rm=TRUE)
# mean(no_mine$Hg_ppm > 0.8, na.rm=TRUE)
#Calculates the proportion of values greater than the US avg for A horizon soils from the USGS reported in Smith et al. (2013), Data Series 801 (0.04 mg/kg)
# mean(USGS_analyze$Hg_ppm > 0.04, na.rm=TRUE)
# mean(no_mine$Hg_ppm > 0.04, na.rm=TRUE)
# #Over the "elevated" level in the Obrist abstract of 0.1 mg/kg
# mean(USGS_analyze$Hg_ppm > 0.1, na.rm=TRUE)

#Calculates the % of values greater than the Canada EPA soil limit reported in Kamunda et al. (2016) (100 mg/kg)
# mean(USGS_analyze$Ni_ppm > 100, na.rm=TRUE)
# mean(no_mine$Ni_ppm > 100, na.rm=TRUE)
# Calculates the proportion of values greater than the US avg for A horizon soils from the USGS reported in Smith et al. (2013), Data Series 801 (18.5 mg/kg)
# mean(USGS_analyze$Ni_ppm > 18.5, na.rm=TRUE)
# mean(no_mine$Ni_ppm > 18.5, na.rm=TRUE)

#Calculates the % of values greater than the Canada EPA soil limit reported in Kamunda et al. (2016) (200 mg/kg)
# mean(USGS_analyze$Pb_ppm > 200, na.rm=TRUE)
# mean(no_mine$Pb_ppm > 200, na.rm=TRUE)
# Calculates the proportion of values greater than the US avg for A horizon soils from the USGS reported in Smith et al. (2013), Data Series 801 (22.2 mg/kg)
# mean(USGS_analyze$Pb_ppm > 22.2, na.rm=TRUE)
# mean(no_mine$Pb_ppm > 22,2, na.rm=TRUE)
```


```{r}
### Cr vs Ni plot R4 asked about
Cr_vs_Ni <- ggplot(data=USGS_analyze, aes(x= Cr_ppm, y= Ni_ppm, color = Source))+
  geom_point()+
  theme_classic()
Cr_vs_Ni

Cr_vs_Ni_2 <- ggplot(data=USGS_analyze, aes(x= Cr_ppm, y= Ni_ppm))+
  geom_point()+
...
% over limits/averages by depth - we decided this was too complex. 
```{r}
#Calculates the % of samples in the dataset that were collected between 0-10cm depth and 0-5cm depth - used to determine which soil comparison to use (we have one for A horizon and one for 0-5cm form USGS from US soils)
mean(USGS_analyze$TOP_CM < 10, na.rm = TRUE)
#Returns 0.735
mean(USGS_analyze$TOP_CM < 5.1, na.rm = TRUE)
#Returns 0.714, due to unit conversion of a group of samples collected ~2in to cm, there is a chunk of samples that have a depth of 5.08cm - which is not meaningfully different than 5.0cm

compare.upper <- filter(USGS_analyze, TOP_CM < 10.1)
#913 observations (set to 10.1 to be inclusive of the 10cm samples)
compare.lower <- filter(USGS_analyze, TOP_CM > 10)
#238 observations

#has percent over health limits/over averages for all samples. 
#Using Canada health standards
all.data.health <- data.frame(metal= rep(c("As", "Cr", "Hg", "Ni", "Pb"), 3),
                  limit=rep(c("all samples","< 10cm", "> 10cm"),each = 5),
                  #first calulates % over health for all data
                  percent = c(mean((USGS_analyze$As_ppm > 20)*100, na.rm=TRUE), mean((USGS_analyze$Cr_ppm > 250)*100, na.rm=TRUE), mean((USGS_analyze$Hg_ppm > 0.8)*100, na.rm=TRUE), mean((USGS_analyze$Ni_ppm > 100)*100, na.rm=TRUE), mean((USGS_analyze$Pb_ppm > 200)*100, na.rm=TRUE),
                  #then calculates % over health for upper soils
                  mean((compare.upper$As_ppm > 20)*100, na.rm=TRUE), mean((compare.upper$Cr_ppm > 250)*100, na.rm=TRUE), mean((compare.upper$Hg_ppm > 0.8)*100, na.rm=TRUE), mean((compare.upper$Ni_ppm > 100)*100, na.rm=TRUE), mean((compare.upper$Pb_ppm > 200)*100, na.rm=TRUE),
                  #then calculates % over health for lower soils
                  mean((compare.lower$As_ppm > 20)*100, na.rm=TRUE), mean((compare.lower$Cr_ppm > 250)*100, na.rm=TRUE), mean((compare.lower$Hg_ppm > 0.8)*100, na.rm=TRUE), mean((compare.lower$Ni_ppm > 100)*100, na.rm=TRUE), mean((compare.lower$Pb_ppm > 200)*100, na.rm=TRUE)))

health_plot <- ggplot(data=all.data.health, aes(x=metal, y=percent, fill=limit)) +
  geom_bar(stat="identity", position=position_dodge()) + ylim(0,100) +
  geom_text(aes(label=round(percent, 1)), vjust=-0.2, color="black",
            position = position_dodge(0.9), size = 3.25, fontface = "bold") +
  scale_fill_manual(values=c("#d73027", "#fee090", "#91bfdb"))+
  ylab("Percent of sample locations over health limit")+
  theme_classic() +
  theme(axis.title.x=element_blank(),
        axis.title.y=element_text(face="bold"),
        axis.text.x = element_text(size = 10, face = "bold"),
...
all.data.avg <- data.frame(metal= rep(c("As", "Cr", "Hg", "Ni", "Pb"), 3),
                limit=rep(c("all samples","< 10cm", "> 10cm"),each = 5),
                #first caluclates at % over US avg
                percent = c(mean((USGS_analyze$As_ppm > 6.6)*100, na.rm=TRUE), mean((USGS_analyze$Cr_ppm > 37)*100, na.rm=TRUE), mean((USGS_analyze$Hg_ppm > 0.04)*100, na.rm=TRUE), mean((USGS_analyze$Ni_ppm > 18.5)*100, na.rm=TRUE), mean((USGS_analyze$Pb_ppm > 22.2)*100, na.rm=TRUE),
                #then calculates at % over US avg
                mean((compare.upper$As_ppm > 6.6)*100, na.rm=TRUE), mean((compare.upper$Cr_ppm > 37)*100, na.rm=TRUE), mean((compare.upper$Hg_ppm > 0.04)*100, na.rm=TRUE), mean((compare.upper$Ni_ppm > 18.5)*100, na.rm=TRUE), mean((compare.upper$Pb_ppm > 22.2)*100, na.rm=TRUE),
               #then calculates % over US avg
               mean((compare.lower$As_ppm > 6.6)*100, na.rm=TRUE), mean((compare.lower$Cr_ppm > 37)*100, na.rm=TRUE), mean((compare.lower$Hg_ppm > 0.04)*100, na.rm=TRUE), mean((compare.lower$Ni_ppm > 18.5)*100, na.rm=TRUE), mean((compare.lower$Pb_ppm > 22.2)*100, na.rm=TRUE)))

average_plot <- ggplot(data=all.data.avg, aes(x=metal, y=percent, fill=limit)) +
  geom_bar(stat="identity", position=position_dodge()) + ylim(0,100) +
  geom_text(aes(label=round(percent, 1)), vjust=-0.2, color="black",
            position = position_dodge(0.9), size = 3.25, fontface = "bold") +
  scale_fill_manual(values=c("#d73027", "#fee090", "#91bfdb"))+
  ylab("Percent of sample locations over US average")+
  theme_classic() +
  theme(axis.title.x=element_blank(),
        axis.title.y=element_text(face="bold"),
        axis.text.x = element_text(size = 10, face = "bold"),
