...
USGS2 <- filter(data1, is.na(DEPTH) == FALSE | is.na(SAMPLE_SOURCE) == FALSE)
...
#filter(str_detect(AKgeochem_3$DESCIRPT, "*;")) this doesn't work - trying to figure out how to tell R to remove rows that don't have ";" since this is what separates depths from descriptions in this column - separating the columns without removing rows without depths produces weird results

##then separate DESCRIPT into depth and location notes - only do once rows without depths removed
AKgeochem_4 <-transform(AKgeochem_3, DESCRIPT = colsplit(DESCRIPT, split = ";", names = c('DEPTH', 'NOTES')))


```

To do once AKgeochem is finished: Filtering down to shared variables between USGS and National Geochem Survey
Both have: Latitude, longitude, soil horizon, datum, depth (once AKgeochem is filtered properly), location description, chemistry data, analytical method, identifiers, collection dates
