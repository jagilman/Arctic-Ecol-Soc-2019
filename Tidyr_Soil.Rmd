...
```{r}
Main_matrix_metals <- select(USGS_analyze, As_ppm, Cr_ppm, Hg_ppm, Ni_ppm, Pb_ppm)
##removing NAs - can't do a PCA w/ NAs
Main_matrix_metals <- filter(Main_matrix_metals, !is.na(As_ppm))
Main_matrix_metals <- filter(Main_matrix_metals, !is.na(Cr_ppm))
Main_matrix_metals <- filter(Main_matrix_metals, !is.na(Hg_ppm))
Main_matrix_metals <- filter(Main_matrix_metals, !is.na(Ni_ppm))
Main_matrix_metals <- filter(Main_matrix_metals, !is.na(Pb_ppm))
...
#princomp runs the PCA, I am telling it not to center/scale columns by setting cor= FALSA and to generate PCA scores with scores = TREUE. 
metals.pca <- princomp(Main_matrix_metals, cor = FALSE, scores= TRUE)
